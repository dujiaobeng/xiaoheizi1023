getgenv()._7461pro = "红脚本 坤" --> 不要修改, 背被踢
(function(define)
	repeat
		game:GetService("RunService").Heartbeat:wait()
	until game:IsLoaded();
	local function check_exploit() --> 检查注入器配置, 如果不通过就不会运行脚本
		if not getgenv then
			return false;
		end
		return true;
	...
	local check_whitelist = function()  --> 检查白名单的功能
	whitelisted = true;
	--[[
		local plr = game:GetService("Players").LocalPlayer;
		table.foreach(whitelist_table, function (i, v)
			if v == plr.Name then --> 如果玩家用户名等于云端白名单列表的名字就能加载脚本
				whitelisted = true;
			end
		end)
		]]
	...
	local function checkModify()
		local develop = {   -->  开发者游戏用户名, 可以免去白名单
			"lixiang_c4",  --> 双引号里面的内容可以改, 需要填写游戏用户名
			"MIXI_SERVICE1",
			"MIXI_SERVICE2",
			"MIXI_SERVICE3",
			"MIXI_SERVICE4",
		...
		--↓防止被别人修改代码的检查
		if (getgenv()._7461pro ~= "红脚本 坤" or not getgenv().zhanghuihuihui) and not isDev() then
			plr:Kick("请不要修改代码");
			task.wait(.01);
       -- while true do end
		end
